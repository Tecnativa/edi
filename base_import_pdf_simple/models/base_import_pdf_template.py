...
class BaseImportPdfTemplate(models.Model):
    ...
    def button_preview(self):
        ...
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "base_import_pdf_simple.action_wizard_base_import_pdf_preview"
        )
    ...
    def _get_items_from_model(self, model):
        return self.search([("model", "=", model)])
    ...
    def _get_table_info_data(self, text):
        ...
        for child_line in child_lines:
            start_lines_content = re.findall(child_line.pattern, text)
            data_column = []
            for start_line_content in start_lines_content:
                if isinstance(start_line_content, tuple):
                    data_column.append(start_line_content[1].strip())
                else:
                    data_column.append(start_line_content.strip())
...
class BaseImportPdfTemplateLine(models.Model):
    ...
    def _process_value(self, value):
        ...
        # Search and return the record if found
        if self.search_field_id:
            if self.search_subfield_id:
                record = self.env[self.search_field_relation].search(
                    [(self.search_subfield_id.name, "=", value)], limit=1
                )
                if record:
                    return record[self.field_name]
            else:
                record = self.env[self.field_relation].search(
                    [(self.search_field_name, "=", value)], limit=1
                )
                if record:
                    return record
